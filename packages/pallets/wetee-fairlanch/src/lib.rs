...
pub mod pallet {
    ...
    #[pallet::storage]
    #[pallet::getter(fn to_staking_total)]
    pub type ToStakingTotal<T: Config> = StorageValue<_, u32, ValueQuery>;
    ...
    #[pallet::storage]
    #[pallet::getter(fn staking_quota)]
    pub type StakingQuota<T: Config> = StorageMap<_, Identity, WeAssetId, BalanceOf<T>, ValueQuery>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn v_staking(
            ...
            Self::staking_asset(who, asset_id, amount)?;
        ...
        pub fn set_economics(
            ...
            let _ = StakingQuota::<T>::set(asset_id, quota);
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn check_epoch(n: BlockNumberFor<T>) -> (BalanceOf<T>, u128, BalanceOf<T>) {
            ...
            if new_epoch == curr_epoch {
                return (pre_reward, curr_epoch, curr_reward);
            }
        ...
        pub fn staking_asset(
            ...
            let mut camount = amount.clone();
            // 记录下个 epoch 的质押数据
            let to_staking = ToStakings::<T>::get(user.clone(), asset_id);
            if to_staking.is_some() {
                camount += to_staking.unwrap();
            }
            let _ = ToStakings::<T>::insert(&user, asset_id, camount);

            // 获取当前的质押数据
            let user_reward = UserNextReward::<T>::get(user.clone());
            if user_reward == 0u32.into() {
                let next_block = Self::get_user_next_reward();
                // 触发下一次奖励
                let _ = NextStakingRewards::<T>::insert(next_block.clone(), user.clone(), true);
